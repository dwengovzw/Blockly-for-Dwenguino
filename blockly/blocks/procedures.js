...
//********************Typed test*********************//
Blockly.Blocks['procedures_mutatorarg_typed'] = {
  /**
   * Mutator block for procedure argument.
   * @this Blockly.Block
   */
  init: function() {
    ...
    this.appendDummyInput()
        ...
        .appendField(field, 'NAME');
    this.appendDummyInput()
        .appendField(MSG.with_type)
        .appendField(new Blockly.FieldDropdown(profile.dwenguino.digital), 'TYPE')
    this.setPreviousStatement(true);
    this.setNextStatement(true);
    this.setColour(Blockly.Blocks.procedures.HUE);
    this.setTooltip(Blockly.Msg.PROCEDURES_MUTATORARG_TOOLTIP);
...
Blockly.Blocks['procedures_defnoreturn_typed'] = {
  ...
  init: function() {
    ...
    this.appendDummyInput()
        ...
        .appendField(nameField, 'NAME')
        .appendField('', 'PARAMS');
    this.setMutator(new Blockly.Mutator(['procedures_mutatorarg_typed']));
    if ((this.workspace.options.comments ||
         (this.workspace.options.parentWorkspace &&
          this.workspace.options.parentWorkspace.options.comments)) &&
        Blockly.Msg.PROCEDURES_DEFNORETURN_COMMENT) {
  ...
  decompose: function(workspace) {
    ...
    var connection = containerBlock.getInput('STACK').connection;
    for (var i = 0; i < this.arguments_.length; i++) {
      var paramBlock = workspace.newBlock('procedures_mutatorarg_typed');
      paramBlock.initSvg();
      paramBlock.setFieldValue(this.arguments_[i], 'NAME');
      // Store the old location.
      paramBlock.oldLocation = i;
  ...
  renameVar: function(oldName, newName) {
    ...
    if (change) {
      ...
      if (this.mutator.isVisible()) {
        var blocks = this.mutator.workspace_.getAllBlocks();
        for (var i = 0, block; block = blocks[i]; i++) {
          if (block.type == 'procedures_mutatorarg_typed' &&
              Blockly.Names.equals(oldName, block.getFieldValue('NAME'))) {
            block.setFieldValue(newName, 'NAME');
          }
        }
