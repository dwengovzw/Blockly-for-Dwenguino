...
let runESToolboxRecording = async (browser, page) => {
  const timeout = 10000;
...

  {
    ...
    }
    {
        ...
        await targetPage.goto("http://localhost:12032/simulator?lang=es");
    ...
    {
        const targetPage = page;
        const element = await waitForSelectors([["#blockly\\:a > div.blocklyTreeRow.blocklyTreeSelected"]], targetPage, { timeout, visible: true });
        await scrollIntoViewIfNeeded(element, timeout);
        await element.click({
          offset: {
            x: 79,
