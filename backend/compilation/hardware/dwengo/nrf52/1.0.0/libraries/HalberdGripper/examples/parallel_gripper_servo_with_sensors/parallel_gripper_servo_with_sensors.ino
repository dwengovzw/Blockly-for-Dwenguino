/**
 * Parallel gripper example for the HalberdGripper library using the
 * HalberdGripperSensor module for real pressure and distance readings.
 *
...
#define GRIPPER_NAME "gripper-left-sensors"

// Gripper geometry
#define WIDTH_MIN 0.000f       // 0 mm
#define WIDTH_MAX 0.079f       // 79 mm
#define MAX_SPEED 0.15f        // m/s, informational
#define LINKAGE_LENGTH 0.03f   // 30 mm, length of the linkages in the gripper.

// Servo angles corresponding to fully closed / fully open.
#define ANGLE_CLOSED 88.8f
...
// Sensor channel ids exposed to airo-mono.
#define SENSOR_ID_TOF       0
#define SENSOR_ID_PRESSURE  1

HalberdGripper gripper;
Servo finger1Servo;
Servo finger2Servo;

HalberdGripperSensor sensors(PIN_A0, Wire);
...
void setup() {
  ...
  if (!sensors.begin()) {
    ...
    Serial.print("Wire init failed: stage=");
    Serial.print(sensors.tof().lastErrorStage());
    Serial.print(", wire_error=");
    Serial.print(sensors.tof().lastWireError());
    Serial.print(", model_id=0x");
    Serial.println(sensors.tof().lastModelId(), HEX);
  } else {
    Serial.println("VL53L4CD detected on Wire.");
  }
  ...
  gripper.configureSensor(SENSOR_ID_TOF, "tof", "mm", 0.0f, 1200.0f);
  gripper.configureSensor(SENSOR_ID_PRESSURE, "pressure", "V", 0.0f, 3.6f);
...
void loop() {
  ...
  {
    uint16_t newDistanceMm;
    if (sensors.readDistance(newDistanceMm)) {
      distanceMm = newDistanceMm;
      distanceValid = true;
    }
    pressureRaw = sensors.readPressureRaw();
    pressureVoltage = sensors.readPressureVoltage();
  }

  Serial.print("Pressure: ");Serial.print(pressureVoltage, 3);
  Serial.print(" V (raw ");Serial.print(pressureRaw);Serial.print(")");
  ...
  gripper.reportSensor(SENSOR_ID_PRESSURE, pressureVoltage);
  if (distanceValid) {
    gripper.reportSensor(SENSOR_ID_TOF, (float)distanceMm);
  }
  gripper.update();
